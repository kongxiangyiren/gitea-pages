...
function build() {
  ...
  // 编译
  execSync(
    'cd ./view/vue-pages && npm run build && cd ../../ && npm run format && npx tslint "src/**/*.ts" --fix && npx tsc',
    {
      stdio: 'inherit'
    }
  );
  ...
  fs.cpSync('./view/404.html', './dist/view/404.html', { recursive: true });
  fs.cpSync('./view/index.html', './dist/view/index.html', { recursive: true });
